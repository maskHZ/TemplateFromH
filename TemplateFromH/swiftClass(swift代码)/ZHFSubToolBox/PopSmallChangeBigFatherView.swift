...
class PopSmallChangeBigFatherView: UIView ,UIGestureRecognizerDelegate{
    //白色view用来装一些控件
    @objc var WhiteView: UIView =  UIView()
    @objc var whiteViewStartFrame: CGRect = CGRect.init(x: ScreenWidth/2 - 10, y: ScreenHeight/2 - 10, width: 20, height: 20)
    @objc var whiteViewEndFrame: CGRect = CGRect.init(x: 40, y: 100, width: ScreenWidth - 80, height: ScreenHeight - 230)
    //取消按钮
    @objc var cancelBtn: UIButton = UIButton()
    //背景区域的颜色和透明度
    @objc var backgroundColor1:UIColor  = UIColor.init(red: 0, green: 0, blue: 0, alpha: 0.4)
    @objc var defaultTime:CGFloat = 0.5
    
    //初始化视图
    @objc func initPopBackGroundView() -> UIView {
